...
) where T = Leaf{T}(value, parent, Vector{Function}(callback_enter), Vector{Function}(callback_exit))
...
function equal(leaf_a::Vector{<:Leaf}, leaf_b::Vector{<:Leaf})
    if length(leaf_a) === length(leaf_b)
        for (leaf_a_item, leaf_b_item) in zip(leaf_a, leaf_b)
